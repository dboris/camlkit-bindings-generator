...
let objc_type =
  let pp fmt t = Format.fprintf fmt "%s" (Encode.string_of_objc_type t)
  and equal t1 t2 =
...
let test_parse_int () =
  let expected = `Int and actual = Encode.parse_type "i" in
...
let test_parse_ptr_float () =
  let expected = `Pointer `Float and actual = Encode.parse_type "^f" in
  A.check A.bool "Result is Option.some" true (Option.is_some actual);
  A.check objc_type "same type" expected (Option.get actual)
...
let test_encode_array () =
  ...
  and actual =
    Encode.parse_type "[10S]" |> Option.map Encode.string_of_objc_type
  in
  A.check A.bool "Result is Option.some" true (Option.is_some actual);
...
let test_encode_ptr_unknown_structure () =
  ...
  and actual =
    Encode.parse_type "^{?}" |> Option.map Encode.string_of_objc_type
  in
  A.check A.bool "Result is Option.some" true (Option.is_some actual);
  A.check A.string "same string" expected (Option.get actual)
...
let test_encode_structure () =
  ...
  and actual =
    Encode.parse_type "{_CFRange=qq}" |> Option.map Encode.string_of_objc_type
  in
  A.check A.bool "Result is Option.some" true (Option.is_some actual);
  A.check A.string "same string" expected (Option.get actual)
...
let test_parse_type_with_modifier () =
  let expected = "string"
  and actual =
    Encode.parse_type "r*" |> Option.map Encode.string_of_objc_type
  in
  A.check A.bool "Result is Option.some" true (Option.is_some actual);
  A.check A.string "same string" expected (Option.get actual)
...
let test_parse_array_of_struct () =
  ...
  and actual =
    Encode.parse_type
      "{CVPlanarPixelBufferInfo=\"componentInfo\"[1{CVPlanarComponentInfo}]}"
    |> Option.map Encode.string_of_objc_type
  in
  A.check A.bool "Result is Option.some" true (Option.is_some actual);
  A.check A.string "same string" expected (Option.get actual)
...
let test_ret_type_conv () =
  ...
  and ret_ty = "llong" in
  let typ =
    Normal (Printf.sprintf "%s @-> returning %s" "int" ret_ty, ret_ty, args)
  in
  let expected =
    "let hello x self = msg_send ~self ~cmd:(selector \"hello:\") ~typ:(int \
     @-> returning llong) x |> LLong.to_int"
...
let suite =
  [
    ...
    ( "parse and encode ptr to unknown structure",
      `Quick,
      test_encode_ptr_unknown_structure );
    ( "parse and encode ptr to varargs structure",
      `Quick,
      test_encode_ptr_varargs_structure );
