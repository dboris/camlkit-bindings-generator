...
let tag_name_to_type name =
  ...
  and append_dot_t str =
    if String.begins_with_char '(' str || not (is_upper (String.get str 0)) then
      str
    else str ^ ".t"
  in
  remove_leading_underscores name |> apply_type_exceptions |> append_dot_t

let rec string_of_objc_type ?(raise_on_struct = false) ty =
  match ty with
  | `Id -> "id"
  | `Class -> "_Class"
  | `Sel -> "_SEL"
  ...
  | `Struct (tag_opt, _fields) -> (
      ...
      match raise_on_struct with
      | true -> raise (Encode_struct (Option.get tag_opt |> tag_name_to_type))
      | false -> Option.get tag_opt |> tag_name_to_type)
  | `Array (_n, t) ->
      (* A C array type is a pointer *)
      "(ptr " ^ string_of_objc_type t ^ ")"
  | `Union (tag_opt, _fields) ->
      (* string option * (string option * t) list *)
      if Option.is_some tag_opt then Option.get tag_opt |> tag_name_to_type
      else raise (Encode_type "Missing tag")
  | `Method (args, ret) -> (
      match args with
      | [ _self; _cmd ] -> "returning (" ^ string_of_objc_type ret ^ ")"
      | _ ->
          (args |> List.tl (* skip self *) |> List.tl (* skip cmd *)
