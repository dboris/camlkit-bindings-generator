...
module String = struct
  ...
  let begins_with_char ch str = Char.equal (String.get str 0) ch
end
...
let apply_type_exceptions ?(allow_underscore = true) = function
  ...
  | "xmlNode" | "xmlParserCtxt" | "xmlElementContent" | "xmlTextReader"
  | "xmlDoc" | "NSRefCountedRunArray" | "NSProgressFraction" | "NSSlice"
  | "SKCAction" | "NSAppleEventManagerSuspension" | "opaque_pthread_mutex_t"
  | "CGImageProvider" | "opaqueCMSampleBuffer" | "EPolygonList"
  | "MRCDescriptor" | "sFILE" | "CCColorProfileContext" | "CCCharBox"
  | "CCBigBox" | "CCBox" | "FSRef" | "CCPulseWindowContext" | "rgbaColor"
  | "rgbMinMaxU8" | "rgbMinMaxFloat" | "CGImageSource" | "CGImageMetadata"
  ...
  | ( "NLConstraintParameters" | "GLKMatrix2" | "GLKMatrix3" | "GLKMatrix4"
    | "GLKVector2" | "GLKVector3" | "GLKVector4" | "CCRange" | "vec2" | "vec3"
    | "vec4" | "Texture" | "Rectangle" | "Options" | "Point"
    | "NSKeyValueCodingControllerModeType"
    | "NSToolbarFullScreenContentViewLayout_t"
    | "CIPredictionModelImageFeatures" | "Geometry2D_rect2D_"
    | "UIRectCornerRadii" | "UITableConstantsBackgroundProperties"
    | "UIPeripheralAnimationGeometry" | "ITColor" | "mach_right_send"
    | "PUSimpleIndexPath" | "PXSimpleIndexPath" | "PHDisplayVelocity"
    | "PXAssetMediaTypeCount" | "PXAssetBadgeInfo" | "PUGridCoordinates"
  ...
  | ty ->
      if (not allow_underscore) && String.contains ty '_' then
        raise (Unsupported_type ty)
      else if String.contains ty '<' then "id"
        (* protocol-implementing types are object pointers *)
      else ty
...
let unsupported_method = function
  ...
  | "linearGravityFieldWithVector" | "velocityFieldWithVector" | "setDirection"
  | "setEulerAngles" | "setVelocity" | "addPoint" | "emitPoint" | "setLastPoint"
...
let is_reserved = function
  ...
  | "while" | "with" | "selector" | "id" | "self" | "string" | "float" | "int"
    ->
      true
  | _ -> false
...
let remove_empty str_list =
  ...
  |> List.filter_map @@ fun a ->
     if String.equal a String.empty then Option.none else Option.some a
...
let arg_labels args = args |> List.map (( ^ ) "~") |> String.concat " "
...
let emit_doc_comment ?(search = false) fw symbol =
  if search then
    ...
    Printf.sprintf
      "(** Apple docs: \
       {{:https://developer.apple.com/documentation/%s/%s?language=objc}%s} *)"
...
let load_framework fw =
  if not (String.equal fw "") then
    fw |> String.split_on_char ','
    |> List.iter @@ fun fw ->
       let bundle =
         NSBundle.self |> NSBundleClass.bundleWithPath (new_string fw)
       in
       if is_nil bundle then Printf.eprintf "Framework bundle not found!\n%!"
       else NSBundle.load bundle |> ignore

let class_image class_name =
  let cls = Objc.get_class class_name in
  if is_nil cls then failwith "Class not found"
  else Inspect.Objc.get_image_name cls

let open_modules mod_names =
  if String.(equal mod_names empty) then []
  else String.split_on_char ',' mod_names |> List.map (( ^ ) "open ")
...
let filter_classes_fn filename =
  if String.(equal filename empty) then Fun.const true
  else
    let to_keep =
